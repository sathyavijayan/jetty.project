    ...
    try {
      withEnv(["JAVA_HOME=${ tool "$jdk" }",
        ...
        configFileProvider(
          ...
          sh "mvn -s $GLOBAL_MVN_SETTINGS -DsettingsPath=$GLOBAL_MVN_SETTINGS -fae -Premote-session-tests -Pci -V -B -e -Djetty.testtracker.log=true $cmdline -Dunix.socket.tmp=" +
                     env.JENKINS_HOME
        }
      }
    }
