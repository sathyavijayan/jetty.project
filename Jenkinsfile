          ...
          steps {
            container('jetty-build') {
              timeout( time: 120, unit: 'MINUTES' ) {
                mavenBuild( "jdk11", "clean install -X", "maven3",
...
def mavenBuild(jdk, cmdline, mvnName, consoleParsers) {
  script {
    try {
      withEnv(["JAVA_HOME=${ tool "$jdk" }",
        ...
        configFileProvider(
          ...
          sh "mvn -s $GLOBAL_MVN_SETTINGS -DsettingsPath=$GLOBAL_MVN_SETTINGS -Dmaven.repo.local=.repository -Premote-session-tests -Pci -V -B -e -Djetty.testtracker.log=true $cmdline -Dunix.socket.tmp=" +
                     env.JENKINS_HOME
        }
      }
    }
    ...
    {
      archiveArtifacts artifacts: '**/jetty-maven-plugin/target/it/**',allowEmptyArchive: true
